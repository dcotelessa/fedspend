...
merge_and_cleanup_worktree() {
  ...
  # copies — untracked files get rm'd; tracked+modified files get reverted
  # to HEAD so the merge brings in the worktree's committed version cleanly.
  for _ctx in ".research/contexts/${STORY}.json" ".research/contexts/${STORY}.md"; do
    [[ -f "$_ctx" ]] || continue
    if git ls-files --error-unmatch "$_ctx" >/dev/null 2>&1; then
      git checkout -- "$_ctx" 2>/dev/null || true
    else
      rm -f "$_ctx"
    fi
  done
