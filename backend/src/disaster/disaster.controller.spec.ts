...
describe('DisasterController', () => {
  ...
  beforeEach(() => {
    ...
    controller = new DisasterController(service);
  });
  ...
  it('calls service.getOverview with empty dto when no params', async () => {
    const overviewData = [
  ...
  it('calls service.getOverview with defGroup', async () => {
    const overviewData = [
      { defGroup: 'CA', totalObligated: 300000, totalAwardCount: 15, stateCount: 2, highestPerCapitaState: 'California', highestPerCapita: 1000, coverageGapCount: 1 },
      { defGroup: '01', totalObligated: 50000, totalAwardCount: 3, stateCount: 1, highestPerCapitaState: 'Florida', highestPerCapita: 200, coverageGapCount: 0 },
    ];
    ...
    const result = await controller.getOverview({ defGroup: 'CA' });

    expect(result).toEqual(overviewData);
    expect(service.getOverview).toHaveBeenCalledWith({ defGroup: 'CA' });
  });

  it('calls service.queryStates with fiscalYear', async () => {
    const statesData = [
    ...
    service.queryStates.mockResolvedValue(statesData);

    const result = await controller.queryStates({ fiscalYear: 2024 });

    expect(result).toEqual(statesData);
    expect(service.queryStates).toHaveBeenCalledWith({ fiscalYear: 2024 });
  });

  it('calls service.queryStates with defGroup and fiscalYear', async () => {
    const statesData: any[] = [];
    service.queryStates.mockResolvedValue(statesData);

    const result = await controller.queryStates({ defGroup: 'CA', fiscalYear: 2024 });

    expect(result).toEqual(statesData);
    expect(service.queryStates).toHaveBeenCalledWith({ defGroup: 'CA', fiscalYear: 2024 });
  });

  it('calls service.queryRatios with fiscalYear', async () => {
    const ratiosData = [
    ...
    service.queryRatios.mockResolvedValue(ratiosData);

    const result = await controller.queryRatios({ fiscalYear: 2024 });
  ...
  it('calls service.queryRatios with no fiscalYear', async () => {
    ...
    const result = await controller.queryRatios({});
  ...
  it('calls service.getStateProfile with state code parameter', async () => {
    ...
    service.getStateProfile.mockResolvedValue(profileData);
  ...
  it('returns null for getStateProfile when no data exists', async () => {
    service.getStateProfile.mockResolvedValue(null);
